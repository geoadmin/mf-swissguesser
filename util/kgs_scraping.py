...
#url = http://mf-chsdi3.int.bgdi.ch/main/wsgi/rest/services/all/MapServer/ch.swisstopo.lubis-luftbilder_farbe/19982271021945/extendedHtmlPopup
...
def get_html(id,lang='de'):
    ...
    for layer in layers:
        url = "http://api3.geo.admin.ch/main/wsgi/rest/services/all/MapServer/%s/%s/htmlPopup?lang=%s" % (layer,id,lang)
...
def get_copyrights(filename='swissguesser/static/storymap9/data/copyright.csv'):
    ...
    try:
        ...
        for row in csv_file:
            ...
            kgs_nr =  "kgs_{0:05d}_{1:04d}".format(int(row['NUMMER']),int(row['BILDNR']))   
...
if __name__ == '__main__':
    ...
    try:
        ...
        csvwriter = csv.DictWriter(test_file, delimiter=',', fieldnames=csv_file.fieldnames)
        ...
        for row in csv_file:   # iterates the rows of the file in orders
            ...
            csvwriter.writerow({k:v if v is not None else v for k,v in row.items()})
            print
